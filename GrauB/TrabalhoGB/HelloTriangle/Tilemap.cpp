...
void Tilemap::draw()
{
	...
	for (int i = 0; i < mapHeight; i++)
	{
		for (int j = 0; j < mapWidth; j++)
		{
			...
			int texIDTile = tileset[map[j][i]]; //pega o indice da textura no tileset 
	...
	blockPos.x = xi + pos.x * tileDimensions.x;
	blockPos.y = yi + pos.y * tileDimensions.y;

	//Atualiza a matriz de modelo do tile
	glm::mat4 model = glm::mat4(1); //matriz identidade
	model = glm::translate(model, glm::vec3(blockPos.x, blockPos.y, 0));
	model = glm::scale(model, glm::vec3(tileDimensions.x, tileDimensions.y, 1.0f));
	shader->setMat4("model", glm::value_ptr(model));
	shader->setVec2("offsets", 0, 0);
...
bool Tilemap::tileVerification(int x, int y)
{
