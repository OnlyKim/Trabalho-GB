...
//#endif
	...
	tilemap.initialize("../maps/map4-regular.txt");
	...
	//GLuint texIDino = generateTexture("../textures/dinosolo1.png");
	GLuint texIDino = generateTexture("../textures/dinoanda.png");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Desenho do sprite do dino
		glm::mat4 model = glm::mat4(1); //matriz identidade
		model = glm::translate(model, glm::vec3(tilemap.getBlockPos().x, tilemap.getBlockPos().y, 0));
		model = glm::scale(model, glm::vec3(37.0f, 37.0f, 1.0f));
		shader.setMat4("model", glm::value_ptr(model));

		//Passar para o shader as coordenadas de texturas

		shader.setVec2("offsets", iFrame * dx, iAnimation * dy);

		//Chamada de desenho
		glBindVertexArray(VAODino);
		glBindTexture(GL_TEXTURE_2D, texIDino); //conecta a textura desejada
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	{
		if (posPersonagem.y < tilemap.getMapHeight() - 1)
		{
			posPersonagem.y++;
		}
	}
	...
	{
		if (posPersonagem.x < tilemap.getMapWidth() - 1)
		{
			posPersonagem.x++;
		}
	}
	...
	//Verificação da tile
	if (tilemap.tileVerification(posPersonagem.x, posPersonagem.y) == true)
	{
		posPersonagem.x = 0;
		posPersonagem.y = 0;
	}
